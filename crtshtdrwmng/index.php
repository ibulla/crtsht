...
function adm_crtsht_id(string $raw): ?int {
    $raw = strtolower(trim($raw));
    if ($raw === '') return null;
    if (str_starts_with($raw, '0x')) $raw = substr($raw, 2);
    if (str_starts_with($raw, '00') && strlen($raw) > 2) $raw = substr($raw, 2);
    if (!preg_match('/^[0-9a-f]{1,2}$/', $raw)) throw new RuntimeException('Enter the hex ending only, e.g. 1b for 0x001b.');
    $id = hexdec($raw);
    if ($id < 1 || $id > CRTSHT_TOTAL) throw new RuntimeException('Hex ID must be 01–80.');
    return (int)$id;
}
function adm_crtsht_hex(?int $id): string { return $id ? '0x' . str_pad(dechex($id), 4, '0', STR_PAD_LEFT) : ''; }
function adm_crtsht_short(?int $id): string { return $id ? str_pad(dechex($id), 2, '0', STR_PAD_LEFT) : ''; }
    ...
    if (!adm_csrf_ok((string)($_POST['csrf'] ?? ''))) {
        ...
        try {
            if ($action === 'reservation_status') {
                ...
                $stmt->bind_param('i', $rid); $stmt->execute(); $res = $stmt->get_result(); $row = $res ? $res->fetch_assoc() : null; if ($res) $res->free(); $stmt->close();
                ...
                $stmt->bind_param('i', $rid); $stmt->execute(); $res = $stmt->get_result(); $assignedCount = $res ? (int)($res->fetch_assoc()['total'] ?? 0) : 0; if ($res) $res->free(); $stmt->close();
                ...
                $stmt->bind_param('i', $rid); if (!$stmt->execute()) throw new RuntimeException('Could not delete draw entries.'); $stmt->close();
                ...
                $stmt->bind_param('i', $rid); if (!$stmt->execute() || $stmt->affected_rows !== 1) throw new RuntimeException('Could not delete reservation.'); $stmt->close();
                $db->commit();
                ...
                $assigned = adm_crtsht_id((string)($_POST['crtsht_id'] ?? ''));
                if ($assigned === null) {
                    $stmt = $db->prepare("UPDATE CRTSHT_Draw_Entries e JOIN CRTSHT_Draw_Reservations r ON r.ID=e.ReservationID SET e.AssignedCRTSHT=NULL,e.AssignedAt=NULL,e.Status=CASE WHEN r.Status='paid' THEN 'paid' WHEN r.Status='cancelled' THEN 'cancelled' ELSE 'reserved' END WHERE e.ID=?");
                    if (!$stmt) throw new RuntimeException('Prepare failed.');
                    $stmt->bind_param('i',$eid);
                } else {
                    $stmt = $db->prepare("UPDATE CRTSHT_Draw_Entries e JOIN CRTSHT_Draw_Reservations r ON r.ID=e.ReservationID SET e.AssignedCRTSHT=?,e.AssignedAt=NOW(),e.Status='assigned' WHERE e.ID=? AND r.Status IN ('reserved','paid')");
                    if (!$stmt) throw new RuntimeException('Prepare failed.');
                ...
                if ($assigned !== null && $stmt->affected_rows < 1) throw new RuntimeException('Only active reserved or paid reservations can receive a CRTSHT.');
                $stmt->close();
                $message = $assigned === null ? 'Assignment removed.' : adm_crtsht_hex($assigned) . ' assigned.';
            } elseif ($action === 'entry_add') {
                $rid = filter_var($_POST['reservation_id'] ?? null, FILTER_VALIDATE_INT, ['options'=>['min_range'=>1]]);
                if (!$rid) throw new RuntimeException('Invalid reservation.');
                $db->begin_transaction();
                $stmt = $db->prepare("SELECT DrawBatch,Status FROM CRTSHT_Draw_Reservations WHERE ID=? LIMIT 1 FOR UPDATE");
                if (!$stmt) throw new RuntimeException('Reservation lookup failed.');
                $stmt->bind_param('i',$rid); $stmt->execute(); $res=$stmt->get_result(); $r=$res?$res->fetch_assoc():null; if($res)$res->free(); $stmt->close();
                if (!is_array($r) || !in_array((string)$r['Status'], ['reserved','paid'], true)) throw new RuntimeException('Extra assignments can only be added to active reservations.');
                $count = $db->query("SELECT COUNT(*) total FROM CRTSHT_Draw_Entries e JOIN CRTSHT_Draw_Reservations r ON r.ID=e.ReservationID WHERE r.Status IN ('reserved','paid') FOR UPDATE");
                if (!$count) throw new RuntimeException('Capacity check failed.');
                $used = (int)($count->fetch_assoc()['total'] ?? 0); $count->free();
                if ($used >= CRTSHT_TOTAL) throw new RuntimeException('All CRTSHTs are already allocated/reserved.');
                $batch = (string)$r['DrawBatch'];
                $entryStatus = (string)$r['Status'];
                $stmt = $db->prepare("INSERT INTO CRTSHT_Draw_Entries (ReservationID,DrawBatch,Status,CreatedAt) VALUES (?,?,?,NOW())");
                if (!$stmt) throw new RuntimeException('Could not create extra assignment.');
                $stmt->bind_param('iss',$rid,$batch,$entryStatus); if(!$stmt->execute()) throw new RuntimeException('Could not create extra assignment.'); $stmt->close();
                $db->commit();
                $message = 'Extra assignment field added.';
            }
        } catch (Throwable $e) {
...
<div class="box"><div class="small">DRAW TICKETS / ASSIGN CRTSHT</div><?php foreach($entries as $i=>$e):?><div class="entrybox"><b>#<?=str_pad((string)$e['ID'],4,'0',STR_PAD_LEFT)?><?php if($i >= (int)$detail['Quantity']):?><span class="extra-tag">EXTRA</span><?php endif;?></b><span class="pill <?=adm_status_class($e['Status'])?>"><?=adm_e($e['Status'])?></span><form method="post"><input type="hidden" name="csrf" value="<?=adm_e(adm_csrf())?>"><input type="hidden" name="action" value="entry_assign"><input type="hidden" name="entry_id" value="<?=$e['ID']?>"><label>CRTSHT HEX<div class="hex-input"><span class="hex-prefix">0x00</span><input type="text" maxlength="2" inputmode="text" name="crtsht_id" value="<?=adm_e(adm_crtsht_short(isset($e['AssignedCRTSHT']) ? (int)$e['AssignedCRTSHT'] : null))?>" placeholder="1b" autocomplete="off" spellcheck="false"></div></label><button type="submit"><?=$e['AssignedCRTSHT']?'CHANGE':'ASSIGN'?></button></form><?php if($e['AssignedCRTSHT']):?><a class="small nowrap" target="_blank" href="/crtsht/<?=$e['AssignedCRTSHT']?>">OPEN <?=adm_e(adm_crtsht_hex((int)$e['AssignedCRTSHT']))?> ↗</a><?php endif;?></div><?php endforeach;?>
<?php if(in_array((string)$detail['Status'], ['reserved','paid'], true)):?><form method="post" class="add-extra" onsubmit="return confirm('Add one extra CRTSHT assignment to this reservation?');"><input type="hidden" name="csrf" value="<?=adm_e(adm_csrf())?>"><input type="hidden" name="action" value="entry_add"><input type="hidden" name="reservation_id" value="<?=$detail['ID']?>"><button type="submit">+ ADD EXTRA ASSIGNMENT</button></form><?php endif;?></div>
<div class="box danger-zone"><div class="small">DELETE RESERVATION</div><?php if(in_array((string)$detail['Status'], ['reserved','cancelled'], true)): ?><p class="muted">Deletes this reservation and all dependent draw entries. Paid or assigned reservations are blocked server-side.</p><form method="post" onsubmit="return confirm('Delete <?=adm_e($detail['ReservationCode'])?> and all its draw entries? This cannot be undone.');"><input type="hidden" name="csrf" value="<?=adm_e(adm_csrf())?>"><input type="hidden" name="action" value="reservation_delete"><input type="hidden" name="reservation_id" value="<?=$detail['ID']?>"><label>Type DELETE to confirm<input name="confirm_delete" autocomplete="off" required pattern="DELETE"></label><button type="submit">DELETE COMPLETE RESERVATION</button></form><?php else: ?><p class="muted">This reservation is <?=adm_e((string)$detail['Status'])?> and cannot be deleted.</p><?php endif; ?></div>
</div></div>
<?php else: ?>
<table class="table"><thead><tr><th>Reservation</th><th>Status</th><th>Amount</th><th>Buyer</th><th>Contact</th><th>Tickets</th><th>Assigned</th><th>Created</th></tr></thead><tbody><?php foreach($list as $r):?><tr><td><a href="?id=<?=$r['ID']?>"><b><?=adm_e($r['ReservationCode'])?></b></a><div class="muted">#<?=$r['ID']?> · Draw <?=adm_e($r['DrawBatch'])?></div></td><td><span class="pill <?=adm_status_class($r['Status'])?>"><?=adm_e($r['Status'])?></span></td><td class="nowrap"><strong>CHF <?=adm_e(number_format((float)$r['TotalPrice'],2,'.',"'"))?></strong><div class="muted"><?= $r['Status']==='paid' ? 'PAID' : ($r['Status']==='reserved' ? 'OPEN' : '—') ?></div></td><td><?=adm_e($r['Name'])?><div class="muted"><?=adm_e($r['City'])?> · <?=adm_e($r['Country'])?></div></td><td><a href="mailto:<?=adm_e($r['Email'])?>"><?=adm_e($r['Email'])?></a><div class="muted"><?=adm_e($r['Mobile'])?></div></td><td><?=$r['Quantity']?>× <span class="muted">#<?=adm_e(str_replace(',', ', #', (string)$r['EntryIDs']))?></span></td><td><?php $ids=array_filter(explode(',',(string)$r['AssignedIDs']),fn($v)=>$v!=='-'); echo adm_e(implode(', ',array_map(fn($v)=>adm_crtsht_hex((int)$v),$ids))); ?></td><td class="nowrap"><?=adm_e($r['CreatedAt'])?></td></tr><?php endforeach;?></tbody></table>
<?php endif; ?>
</body></html>