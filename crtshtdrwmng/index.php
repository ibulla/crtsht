...
$summary = ['total'=>0,'reserved'=>0,'paid'=>0,'assigned'=>0,'cancelled'=>0];
...
<?php else: ?>
<table class="table"><thead><tr><th>Reservation</th><th>Status</th><th>Buyer</th><th>Contact</th><th>Tickets</th><th>Assigned</th><th>Created</th></tr></thead><tbody><?php foreach($list as $r):?><tr><td><a href="?id=<?=$r['ID']?>"><b><?=adm_e($r['ReservationCode'])?></b></a><div class="muted">#<?=$r['ID']?> · Draw <?=adm_e($r['DrawBatch'])?></div></td><td><span class="pill <?=adm_status_class($r['Status'])?>"><?=adm_e($r['Status'])?></span></td><td><?=adm_e($r['Name'])?><div class="muted"><?=adm_e($r['City'])?> · <?=adm_e($r['Country'])?></div></td><td><a href="mailto:<?=adm_e($r['Email'])?>"><?=adm_e($r['Email'])?></a><div class="muted"><?=adm_e($r['Mobile'])?></div></td><td><?=$r['Quantity']?>× <span class="muted">#<?=adm_e(str_replace(',', ', #', (string)$r['EntryIDs']))?></span></td><td><?=adm_e((string)$r['AssignedIDs'])?></td><td class="nowrap"><?=adm_e($r['CreatedAt'])?></td></tr><?php endforeach;?></tbody></table>
<?php endif; ?>
</body></html>