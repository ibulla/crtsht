...
function adm_crtsht_id(string $raw): ?int {
    $raw = trim($raw);
    if ($raw === '') return null;
    if (preg_match('/^0x([0-9a-f]+)$/i', $raw, $m)) {
        $id = hexdec($m[1]);
    } elseif (preg_match('/^[0-9]+$/', $raw)) {
        $id = (int)$raw;
    } else {
        throw new RuntimeException('Use a decimal ID (1–128) or hex ID such as 0x001b.');
    }
    if ($id < 1 || $id > CRTSHT_TOTAL) throw new RuntimeException('CRTSHT ID must be 1–128 / 0x0001–0x0080.');
    return (int)$id;
}
    ...
    if (!adm_csrf_ok((string)($_POST['csrf'] ?? ''))) {
        ...
        try {
            if ($action === 'reservation_status') {
                ...
                if (!is_array($r) || (string)$r['Status'] !== 'paid') throw new RuntimeException('Extra assignments can only be added to paid reservations.');
                ...
                $stmt = $db->prepare("INSERT INTO CRTSHT_Draw_Entries (ReservationID,DrawBatch,Status,CreatedAt) VALUES (?,?,'paid',NOW())");
                if (!$stmt) throw new RuntimeException('Could not create extra assignment.');
                $stmt->bind_param('is',$rid,$batch); if(!$stmt->execute()) throw new RuntimeException('Could not create extra assignment.'); $stmt->close();
                $db->commit();
