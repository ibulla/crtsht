...
$title=crt_title($id,$meta); $attrs=crt_attrs($meta); $art=crt_artwork($id); $cake=crt_cake($id); $cid=crt_cid($meta);
$dbrow=crt_db_record($id); $wallet=trim((string)($dbrow['ETH_Adr']??'')); $mint=$wallet!==''?crt_mint_record($wallet):null;
$contract=$mint?(string)($mint['contractAddress']??''):''; $tokenId=$mint?(string)($mint['tokenID']??''):'';
...
$birthday=(int)($attrs['birthday']??0); $jsonCid=trim((string)($dbrow['IPFS_JSON']??'')); $description=trim((string)($meta['description']??''));
...
<div class="row"><span class="label">poo</span><span><?=crt_e($attrs['POO']??'')?></span></div><div class="row"><span class="label">author</span><span><?=crt_e($attrs['AUTHOR']??'iBulla.com')?></span></div>

<div class="section-head">PROVENANCE / ETHEREUM</div>
...
<div class="row"><span class="label">token</span><span><?php if($contract):?><a target="_blank" rel="noopener" href="https://etherscan.io/token/<?=crt_e($contract)?>"><?=crt_e((string)($mint['tokenName']??'MORE'))?><?=($mint['tokenSymbol']??'')!==''?' / '.crt_e((string)$mint['tokenSymbol']):''?> ↗</a><?php endif;?></span></div>
<div class="row"><span class="label">token id</span><span><?=crt_e($tokenId)?></span></div><div class="row"><span class="label">contract</span><span class="value"><a target="_blank" rel="noopener" href="https://etherscan.io/token/<?=crt_e($contract)?>?a=<?=crt_e($tokenId)?>"><?=crt_e($contract)?> ↗</a></span></div>
...
<div class="row"><span class="label">from</span><span><?php if($from):?><a target="_blank" rel="noopener" title="<?=crt_e($from)?>" href="https://etherscan.io/address/<?=crt_e($from)?>"><?=crt_e(short_wallet($from))?> ↗</a><?php endif;?></span></div><div class="row"><span class="label">to</span><span><?php if($to):?><a target="_blank" rel="noopener" title="<?=crt_e($to)?>" href="https://etherscan.io/address/<?=crt_e($to)?>"><?=crt_e(short_wallet($to))?> ↗</a><?php endif;?></span></div><div class="row"><span class="label">confirmations</span><span><?=crt_e((string)($mint['confirmations']??''))?></span></div>
<?php elseif($wallet!==''):?><div class="row"><span class="label">chain data</span><span class="muted">wallet recovered · transaction lookup unavailable</span></div><?php endif;?>

<div class="section-head">PROVENANCE / NETWORK</div>
<?php if($jsonCid):?><div class="row"><span class="label">token uri</span><span class="value copy" data-copy><?=crt_e('ipfs://'.$jsonCid)?></span></div><?php endif;?>
...
<script>document.querySelectorAll('[data-copy]').forEach(el=>el.addEventListener('click',async()=>{try{await navigator.clipboard.writeText(el.textContent.trim());const old=el.textContent;el.textContent='COPIED';setTimeout(()=>el.textContent=old,700)}catch(e){}}));const ipfsStatus=document.getElementById('mooncake-ipfs-status');if(ipfsStatus){const cid=ipfsStatus.dataset.cid;const gateways=[`https://dweb.link/ipfs/${cid}`,`https://ipfs.io/ipfs/${cid}`,`https://w3s.link/ipfs/${cid}`];let i=0;const probe=()=>{if(i>=gateways.length){ipfsStatus.textContent='TOKEN IMAGE / IPFS GATEWAYS UNAVAILABLE';return;}const url=gateways[i++],test=new Image();test.onload=()=>{try{ipfsStatus.textContent='TOKEN IMAGE / IPFS VERIFIED / '+new URL(url).hostname}catch(e){ipfsStatus.textContent='TOKEN IMAGE / IPFS VERIFIED'}};test.onerror=probe;test.src=url;};if('requestIdleCallback'in window)requestIdleCallback(probe,{timeout:1200});else setTimeout(probe,250);}const lb=document.getElementById('lightbox'),lbi=lb.querySelector('img');document.querySelectorAll('.zoomable').forEach(z=>z.addEventListener('click',()=>{lbi.src=z.src;lb.classList.add('open')}));lb.addEventListener('click',()=>lb.classList.remove('open'));document.addEventListener('keydown',e=>{if(e.key==='Escape')lb.classList.remove('open')});</script></body></html>